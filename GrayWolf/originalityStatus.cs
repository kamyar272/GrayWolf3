...
namespace GrayWolf
{
    public partial class originalityStatus : UserControl
    {
        public originalityStatus()
        {
            InitializeComponent();
            loadtext();
        }
        void loadtext()
        {
            ...
            string i1=nextI1(invs);
            this.textBox1.AppendText(i1);
            inv.Close();
            invs.Close();

        }
        string nextI1(StreamReader s)
        {
            string res = "", tmp;
            bool flag = false;
            tmp = NextWord(s);
